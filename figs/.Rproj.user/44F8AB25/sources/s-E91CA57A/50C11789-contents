...
## ----read-polity---------------------------------------------------------
library("foreign")
raw_polity <- read.spss("http://www.systemicpeace.org/inscr/p4v2017.sav",
                         to.data.frame = TRUE)
...
polityCLEAN <- data_state_year %>% select(ccode, year, polity)
write_csv(polityCLEAN, "data/polityCLEAN.csv")
